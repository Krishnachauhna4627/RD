...
export function listProducts(): Promise<ProductRow[]> {
  return query<ProductRow>(
    'SELECT id, name, category, material_type, created_at, updated_at FROM products ORDER BY name ASC',
  );
}

export function findProductById(id: number): Promise<ProductRow | null> {
  return queryOne<ProductRow>(
    'SELECT id, name, category, material_type, created_at, updated_at FROM products WHERE id = ?',
    [id],
  );
}
...
export async function createProduct(
  ...
  const result = await execute(
    'INSERT INTO products (name, category, material_type) VALUES (?, ?, ?)',
    [name, category, materialType],
  );
